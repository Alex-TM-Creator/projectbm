...
import type { Goal, GoalType, PeriodGroup, Role, Period, SalesOrder, SaleType, Service } from "@/lib/definitions";
...
export default function RealizadoFuncaoPage() {
  ...
  const fetchData = React.useCallback(async () => {
    try {
      setLoading(true);
      const [goalsSnap, typesSnap, periodsSnap, rolesSnap, saleTypesSnap, salesOrdersSnap, servicesSnap] = await Promise.all([
  ...
  const handleSaveAll = async (valuesToSave?: { [key: string]: number }) => {
    ...
    // that have roleId and whose value has actually changed.
    for (const goalId in values) {
      const goal = goals.find(g => g.id === goalId);
      if (goal && goal.roleId && (goal.realizado || 0) !== values[goalId]) {
  ...
  const handleCalculateAndFill = () => {
    ...
    goals.forEach(goal => {
      if (goal.periodGroupId !== selectedPeriodGroupId || !goal.roleId) return;
    ...
    <>
      <div className="flex flex-col gap-6">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
            <h1 className="text-3xl font-bold font-headline tracking-tight">Lançar Realizado por Função</h1>
            <p className="text-muted-foreground">Insira os valores realizados para cada meta e função.</p>
          </div>
          ...
          <CardContent>
            {loading ? (
              ...
              <Accordion type="multiple" className="w-full">
                {groupGoals(goals.filter(g => !!g.roleId && !g.userId)).map(group => (
                                ...
                                <TableBody>
                                  {goalsInPeriod.map(goal => {
                                    ...
                                    const target = (isFreight && (() => {
                                      const mercantilGoal = goals.find(g => g.periodId === goal.periodId && g.roleId === goal.roleId && goalTypes.find(t => t.id === g.goalTypeId && t.name.toLowerCase().includes('mercantil')));
