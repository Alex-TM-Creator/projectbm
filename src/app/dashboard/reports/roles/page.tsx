...
export default function RolesReportPage() {
  ...
  const [reportData, setReportData] = React.useState<RoleReportData[]>([]);
  ...
  const processData = React.useCallback(() => {
    ...
    const { goals, goalTypes, roles } = allData;
    ...
    const roleReports = roles.map(role => {
      const roleGoals = goals.filter(g => 
        g.roleId === role.id &&
        !g.userId && // Ensure it's a role goal, not a user's goal who has this role
        targetPeriodIds.has(g.periodId)
      );
      
      const goalTypeTotalsMap: Map<string, Omit<GoalTypeTotals, 'totalAchievement'>> = new Map();

      const processedGoals: ProcessedGoal[] = roleGoals.map(goal => {
        ...
        if (isFreteType(goalType)) {
          const mercantilGoal = roleGoals.find(g => {
    ...
    setReportData(roleReports.filter(r => r.goals.length > 0));
  ...
  const fetchData = React.useCallback(async () => {
    try {
      ...
      setAllData({ goals, goalTypes, roles });
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Acompanhe o desempenho de cada função no grupo
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center h-80 rounded-2xl border border-dashed gap-4">
          ...
          <div className="text-center">
            ...
            <p className="mt-1 text-sm text-muted-foreground">Não há metas para funções no grupo '{selectedGroupName}'.</p>
          </div>
        </div>
      ) : (
        <Accordion type="multiple" className="space-y-5" defaultValue={reportData.map(r => r.role.id)}>
          {reportData.map(({ role, goals, goalTypeTotals }) => {
            ...
            return (
              <AccordionItem value={role.id} key={role.id} className="border-0">
                      ...
                      <div className="flex items-center gap-4">
                        <div className={`p-0.5 rounded-xl ${
                          ...
                          <div className="flex h-12 w-12 items-center justify-center rounded-[10px] bg-background">
                            <BriefcaseBusiness className="h-6 w-6" style={{
                        ...
                        <div className="text-left">
                          <p className="font-bold text-base leading-tight">{role.name}</p>
