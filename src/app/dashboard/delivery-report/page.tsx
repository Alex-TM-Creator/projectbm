...
import type { SalesOrder, AssistanceRequest, Branch, Driver, Team, CompanyBranch } from "@/lib/definitions";
import {
  Card,
  CardContent,
  CardDescription,
...
export default function DeliveryReportPage() {
  ...
  const fetchData = React.useCallback(async () => {
    try {
      ...
      const [
        ...
        getDocs(query(collection(db, "astec"))),
  ...
  const filteredData: ReportItem[] = React.useMemo(() => {
    ...
    const allItems: ({item: SalesOrder | AssistanceRequest, itemType: 'Pedido' | 'ASTEC', itemDate: string, itemNumber: number, status: string})[] = [
      ...allOrders
        .filter(o => ['pending', 'concluido', 'rejeitado'].includes(o.deliveryStatus || 'pending'))
        .map(o => ({item: o, itemType: 'Pedido' as const, itemDate: o.deliveryDate!, itemNumber: o.orderNumber, status: o.deliveryStatus || 'pending'})),
      ...allAstecs
        .filter(a => ['pending', 'in_progress', 'finished'].includes(a.status))
        .map(a => ({item: a, itemType: 'ASTEC' as const, itemDate: a.updatedAt?.toDate ? a.updatedAt.toDate().toISOString() : a.scheduledDate, itemNumber: a.assistanceNumber || 0, status: a.status}))
    ];
    ...
    const results = allItems
      ...
      .map(({item, itemType, itemDate, itemNumber, status}) => {
        ...
        if (!driver) {
          finalFreight = 0;
        } else if (driver.discounts && driver.discounts.length > 0) {
        ...
        const needsStairClimbing = itemType === 'Pedido' && (item as SalesOrder).services?.some(s => s.serviceName.toLowerCase().includes('subida de escada'));
        ...
        return {
          ...
          needsStairClimbing: !!needsStairClimbing,
          status: status,
        };
      });
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold font-headline tracking-tight">Relatório de Entregas</h1>
          <p className="text-muted-foreground">Filtre e visualize os valores de frete a receber.</p>
        </div>
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead>Subida de Escada</TableHead>
              ...
              <TableFooter>
                <TableRow className="text-lg font-bold">
                    <TableCell colSpan={8} className="text-right">Total a Receber</TableCell>
