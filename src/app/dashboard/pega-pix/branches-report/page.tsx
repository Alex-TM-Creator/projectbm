...
import type { PegaPixGoal, PegaPixAward, Branch, GoalLevelTargets, PeriodGroup } from "@/lib/definitions";
...
export default function PegaPixBranchesReportPage() {
  const { toast } = useToast();
  const [results, setResults] = React.useState<BranchResult[]>([]);
  ...
  const processData = React.useCallback(() => {
    ...
    const { goals, awards, branches, periodGroups } = allData;
    ...
    const branchGoals = filteredGoals.filter(g => g.responsibleType === 'branch');
    
    const resultsByBranch = new Map<string, BranchResult>();

    for (const goal of branchGoals) {
        const branch = branches.find(b => b.id === goal.responsibleId);
        if (!branch) continue;

        if (!resultsByBranch.has(branch.id)) {
             resultsByBranch.set(branch.id, {
                branch,
                goals: [],
                totalRealizado: 0,
                achievedLevel: null,
        ...
        const branchResult = resultsByBranch.get(branch.id)!;
        branchResult.goals.push(goal);
        branchResult.totalRealizado += goal.realizado || 0;
    }

    const finalResults = Array.from(resultsByBranch.values()).map(branchResult => {
        const { goals, totalRealizado } = branchResult;
        ...
        const responsibleAward = awards.find(a => 
            a.responsibleType === 'branch' && 
            a.responsibleIds.includes(branchResult.branch.id)
        );
  ...
  const fetchData = React.useCallback(async () => {
    try {
      setLoading(true);
      const [goalsSnap, awardsSnap, branchesSnap, periodsSnap] = await Promise.all([
      ...
      setAllData({ goals, awards, branches, periodGroups } as any);
  ...
  const LeaderboardCard = ({ result, rank }: { result: BranchResult, rank: number }) => {
    const { branch, achievedLevel, awardValue, nextLevel, progressToNextLevel, remainingForNextLevel } = result;
    ...
    return (
        <Card className={cn(
            ...
            <CardContent className="p-6 flex flex-col items-center text-center">
                ...
                <h3 className="text-xl font-bold text-card-foreground">{branch.name}</h3>
  ...
  return (
    <div className="flex flex-col gap-6">
        <div className="relative text-center py-10 md:py-16 rounded-lg overflow-hidden bg-card border">
            ...
            <p className="text-muted-foreground mt-2">Ranking por Filial (Períodos Ativos)</p>
        </div>
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {results.map((result, index) => (
                <LeaderboardCard key={`${result.branch.id}`} result={result} rank={index} />
            ))}
             {results.length === 0 && (
